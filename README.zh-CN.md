...
很多手环类 App 很难测试，因为它们依赖一只真实手环：手环要在身边、要有电、要能配对，还要刚好处在你想测试的状态。MockYourBand 的作用，就是把一台 Android 手机变成一只可控制的“虚拟手环”。

你可以用它来：

- 不带真实手环，也能演示手环 App 的连接和数据展示流程。
- 稳定复现连接失败、数据同步、心率测量等问题。
- 按需要模拟电量、心率、睡眠、设备状态等场景。
- 让产品、测试、客服、研究人员不用理解蓝牙协议，也能看懂 App 的工作过程。
- 做更安全的演示，因为数据是模拟的，可以随时调整，不会影响真实设备或真实用户。

可以把它理解成一个“手环 App 的排练设备”：真实手环不方便、不稳定或不好控制时，用它来完成演示和测试。
